			...
			<div className="form-group">
				{
					(allGenIdeas)?					
						allGenIdeas.map((value, index) => (
							<div className="form-group" style={{cursor:'pointer',border:'1px solid grey'}}>{value.text.substring(0, 100)+"..."} <button className="btn btn-outline-primary m-2" onClick={() => fnSelectedText(value.text)}>Use This One</button> </div>
						))
					:
						null
				}
