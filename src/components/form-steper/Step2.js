...

				{
					(allGenIdeas)?					
						allGenIdeas.map((value, index) => (
							<div className="form-group" style={{cursor:'pointer',border:'1px solid #e8e8e8'}} >{value.text.substring(0, 50)+"..."} <button className="btn btn-outline-primary m-2" onClick={() => fnSelectedText(value.text)}>Use This One</button></div>
						))
					:
						null
				}
